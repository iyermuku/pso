...
Applied to the Michaelewicz function (2D minimization)
"""
...
# Run parameter study
results = []
overall_best_val = np.inf  # for minimization
...
for i, w in enumerate(inertias):
    ...
    for j, (c1, c2) in enumerate(valid_c_pairs):
        ...
        # Find iteration when 99.7% of best is reached (for minimization: closest to best)
        target_val = best_val + (0 - best_val) * 0.003  # 99.7% means 0.3% away
        gbest_history = history['gbest_history']
        iters_to_target = None
        for iter_idx, val in enumerate(gbest_history):
            # For minimization, check if we're within 0.3% of the best
            if abs(val - best_val) <= abs(best_val * 0.003):
        ...
        # Track overall best (lowest value for minimization)
...
ax1.plot(fastest_history, marker='.', linewidth=1.5, color='green')
ax1.axhline(y=fastest_params['best_val'], color='r', linestyle='--', label='Best value found')
ax1.set_title(f"Fastest to 99.7% of Best\nw={fastest_params['w']:.1f}, c1={fastest_params['c1']:.1f}, c2={fastest_params['c2']:.1f}")
ax1.set_xlabel("Iteration")
ax1.set_ylabel("Best Objective Value (Minimization)")
...
if is_different:
    ...
    ax2.axhline(y=overall_best_val, color='r', linestyle='--', label='Best value found')
    ax2.set_title(f"Overall Best\nw={overall_best_params['w']:.1f}, c1={overall_best_params['c1']:.1f}, c2={overall_best_params['c2']:.1f}")
    ax2.set_xlabel("Iteration")
    ax2.set_ylabel("Best Objective Value (Minimization)")
...
top10_val = df.nsmallest(10, 'best_val')[['w', 'c1', 'c2', 'best_val', 'iters_to_99_7']]
