...
def robust_constriction_pso(seed: int, iters: int, swarm: int,
    ...
    for i in range(iters):
        for p in range(swarm):
            J[p], metrics = objective_with_penalty(X[p], alpha_mass, alpha_disp, alpha_stress)
            if J[p] < pbest_J[p]:
                pbest_J[p] = J[p]
                pbest_X[p] = X[p].copy()
            if J[p] < gbest_J:
        ...
        if logger and (i % max(1, iters//10) == 0 or i == iters-1):
            logger.info(f"[robust] iter={i+1}/{iters} J_best={gbest_J:.6f} mass={gbest_metrics['mass']:.4f} disp={gbest_metrics['max_disp']:.6f} stress={gbest_metrics['max_stress']:.6f}")
    return {
        'gbest_J': gbest_J,
        'gbest_X': gbest_X,
...
def single_run_pso(seed: int, iters: int, swarm: int,
    ...
    for i in range(iters):
        ...
        for p in range(swarm):
            areas = X[p, :16]
            Jp, metrics = objective_with_penalty(areas, alpha_mass, alpha_disp, alpha_stress)
            if Jp < pbest_J[p]:
                pbest_J[p] = Jp
                pbest_X[p] = X[p].copy()
            if Jp < gbest_J:
        ...
        if logger and (i % max(1, iters//10) == 0 or i == iters-1):
            logger.info(f"[single] iter={i+1}/{iters} J_best={gbest_J:.6f} mass={gbest_metrics['mass']:.4f} disp={gbest_metrics['max_disp']:.6f} stress={gbest_metrics['max_stress']:.6f} w={w_hist[-1]:.3f} c1={c1_hist[-1]:.3f} c2={c2_hist[-1]:.3f}")
