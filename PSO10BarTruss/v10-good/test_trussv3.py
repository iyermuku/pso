...
def analyze_experiment(exp_name: str, A: List[float], stress_limit: float, disp_limit: float,
                       provided_weight: Optional[float], weight_tol: float) -> Tuple[np.ndarray, np.ndarray, float, str, float, bool, float, float]:
    """Run the truss analysis and assemble status information.
    Returns (stresses, U, mass, status, weight_diff, mismatch_flag,
             stress_violation_amount_ksi, displacement_violation_amount_in).
    """
    ...
    stress_amt = float(np.max(stress_over)) if stress_over.size else 0.0
    disp_amt = float(np.max(disp_over)) if disp_over.size else 0.0

    stress_viol = bool(stress_amt > 0)
    disp_viol = bool(disp_amt > 0)
    if stress_viol and disp_viol:
        status = f"both violated (stress +{stress_amt:.6f} ksi, disp +{disp_amt:.6f} in)"
    elif stress_viol:
        status = f"stress violated (+{stress_amt:.6f} ksi)"
    elif disp_viol:
        status = f"displacement violated (+{disp_amt:.6f} in)"
    else:
    ...
    return stresses, U, mass, status, weight_diff, mismatch_flag, stress_amt, disp_amt
...
def main():
    ...
    if args.in_csv:
        ...
        for exp in exp_names:
            stresses, U, mass, status, wdiff, mismatch, stress_amt, disp_amt = analyze_experiment(
            ...
            for m in range(1, 11):
                ...
                row = {
                    ...
                    "stress_violation_amount_ksi": float(stress_amt),
                    "displacement_violation_amount_in": float(disp_amt),
                }
                results.append(row)
